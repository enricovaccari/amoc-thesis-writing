...
\input{figures/data_pipeline/tab_ae_gap.tex}
\input{figures/data_pipeline/fig_manifold.tex}
...
At first glance, the results appear very strong. The rolling variance of the intensity series shows an increasing trend, with Kendall's $\tau = +0.078$ and a p-value of $2\times10^{-42}$; similarly, the rolling autocorrelation increases with $\tau = +0.088$ and a p-value of $1\times10^{-52}$. Taken directly, these values would suggest an extremely strong statistical signal. However, this interpretation would be misleading.\footnote{Kendall's test assumes that observations are independent. In this analysis, rolling statistics are calculated using heavily overlapping 730-day windows, meaning that consecutive values are highly dependent. This effect is amplified by the strong day-to-day autocorrelation of the original series ($0.943$).}

The reason is that the rolling windows overlap substantially, so the thousands of calculated values do not represent independent observations. This is a common statistical pitfall: the software returns a very small p-value without accounting for the dependence between the input points, making the result appear more certain than it actually is. When the number of independent observations is considered instead --- approximately 19.5 non-overlapping 730-day windows --- and the statistical test is corrected accordingly, the apparent trends disappear. The corrected p-value for the variance trend is \textbf{0.634}, while for the autocorrelation trend it is \textbf{0.595}. Neither result is statistically significant (Table~\ref{tab:ews}). The initial strong signal was therefore caused by treating dependent observations as if they were independent.

\input{figures/data_pipeline/tab_ews.tex}
\input{figures/data_pipeline/fig_ews.tex}

The final conclusion is therefore a \emph{null result}. Once the dependence structure of the data is properly considered, the classical early-warning indicators cannot be distinguished from noise. This supports H3 in its null form and represents an important methodological contribution rather than a limitation. A positive result obtained by incorrectly interpreting the statistics would have been more appealing, but it would not have been reliable. Demonstrating that an apparently strong signal disappears when statistical dependence is properly addressed reflects the methodological care at the core of this thesis.

% -------------------------------------------------------------------------
\section{Anomaly Detection}
\label{sec:detect-anomalies}

Early-warning analysis looks for slow \emph{trends}. A different question is about individual \emph{events}: which moments depart from the normal behaviour of the system. An Isolation Forest applied to the daily PC1--PC2 coordinates identifies \SI{5}{\percent} of the record, corresponding to 730 days, as anomalous. Looking at when these days occur reveals an important pattern: they are concentrated in 2009--2010 (158 days), 2012--2013 (100 days), and 2018 (66 days). However, the anomaly score does not increase over time. The unusual moments are distributed across the record rather than accumulating toward the end, providing another indication that the system is not showing a progressive movement toward a threshold.

\input{figures/data_pipeline/fig_anomalies.tex}

Two aspects are important for this thesis. First, this analysis supports the null result from a completely independent perspective: an algorithm that does not rely on early-warning theory also finds no gradual shift, but rather isolated events distributed across the twenty-year record. It reaches the same conclusion through a different method.\footnote{The anomaly clusters correspond to independently identified features --- the 2009--2010 weakening and the variance peaks of 2012--2013 and 2018 --- with \SI{41.4}{\percent} of anomalous days occurring in the highest quarter of local variance. Different methods reveal the same physical behaviour.} Second, these anomalies provide the basis for a specific design decision. Following the grammar introduced in Chapter~\ref{ch:literature}, departures from the normal regime can be translated into local changes in perceptual texture (principle~P3). The moments identified by the detector are therefore the same moments that the experiential representation will transform into perceptible changes --- meaning that anomaly detection is not only a result about the AMOC, but also a direct input for how the AMOC is represented.

% -------------------------------------------------------------------------
\section{Observation Against Reanalysis}
\label{sec:detect-copernicus}

The analysis so far has relied on RAPID observations. The final step compares them with the Copernicus GREP reanalysis ensemble over the 237 months they share. This comparison serves two purposes: characterising how well the reanalysis reproduces the observed signal, and identifying the uncertainty that the \emph{translate} phase needs to communicate. The two datasets show moderate agreement. Their correlation is 0.754, with a small bias of $+0.584$~Sv (RAPID being slightly stronger) and a root-mean-square difference of 2.300~Sv. Both remain close to 17~Sv and show similar variations over time. In other words, the reanalysis captures the overall behaviour of the circulation, even when it does not reproduce the exact value of each individual month.

%\input{figures/data_pipeline/fig_rapid_copernicus.tex}
...
The analytical phase provides the \emph{translate} phase with a documented substrate rather than an assumed one. The vertical structure of the AMOC can be represented through two interpretable coordinates that explain \SI{97.86}{\percent} of its variability: intensity and vertical redistribution. These coordinates are not only a convenient simplification but a faithful representation of the system: a non-linear model confirms that the underlying manifold is effectively flat, meaning that the transformation does not introduce relevant distortion. The record does not show robust early-warning trends, but it does contain clear discrete anomalies that can become perceptual texture. The comparison with the reanalysis also provides an uncertainty estimate that is physically meaningful, although too narrow, and therefore ready to be translated into perception. These findings are not only descriptions of the ocean system. Each one becomes a justified input for the next phase: a compact, faithful, interpretable, event-aware, and uncertainty-carrying substrate that allows the \emph{translate} phase to make an invisible system perceptible without misrepresenting it.