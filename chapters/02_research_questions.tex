...
Time is the binding constraint, with completion planned for late August 2026. The theoretical and analytical writing is front-loaded so that the riskiest part --- implementing the experiential condition --- is undertaken with the surrounding text stable. Scope control is treated as a methodological decision: forecasting is deferred so the thesis stays on representation rather than prediction \citep{sonnewald2021}; the prototype is a research artefact, not a product; and the study is an exploratory comparison, qualitative-primary and not statistically powered. Two flexibility mechanisms protect coherence: if the study slips, the analytical and design chapters proceed independently; and if the experiential build hits a technical limit, the design grammar remains a contribution in itself (O2.2), defensible through the simpler conditions. The thesis is ambitious but focused --- each objective matched to the available time, each claim to the evidence it can bear.
