...
\textls[50]{Sample Composition · Descriptive Analysis · Coding Outcomes · Structural
Comprehension · Misconception Frequency · Confidence vs Correctness · Emergent Codes}
\end{tabular}
}

\vspace{0.5cm}

This chapter reports what the coded responses show: the sample, the coding outcome for each scene, the frequency of the streamfunction misconception, the relationship between confidence and correctness, and the distribution of the emergent codes across conditions. Their interpretation, and their relation to the expectations of Chapter~\ref{ch:userstudy}, is left to Chapter~\ref{ch:discussion}. Given the sample of twenty-five, all figures are descriptive; the three exploratory tests below are labelled as such and are not treated as inferential. Effect sizes and confidence intervals are not reported: at twelve and thirteen participants per condition their intervals would span almost the whole parameter space, and presenting them would lend an inferential appearance to a comparison this chapter does not make. The artefact whose reception is reported here is documented in Chapter~\ref{ch:translate}.\footnote{All counts and tables in this chapter were recomputed directly from the coded response set, which is held in the analytical repository documented in \S\ref{app:c-repos}.}
...
A balance check compared the two conditions on these measures (Figure~\ref{fig:balance}). The experimental group was not higher than the control group on any of them, and was slightly lower on all three; the largest gap was in data-visualisation familiarity (5.54 against 6.92 on a ten-point scale, a difference of 1.38 points).\footnote{A Mann--Whitney comparison places that gap at $p \approx 0.09$, reported here only for completeness. The size and direction of a baseline imbalance, not its statistical significance, are what matter for interpretation: a significance test on baseline balance answers a question the randomisation has already settled.} The direction matters: it runs the way that would make the Scene~I misconception more likely in the experimental condition (\S\ref{sec:res-misconception}), and it is returned to in Chapter~\ref{ch:discussion}.

% TODO(Enrico): report the Italian/English split of the sample, overall and by condition.
% Four questionnaire variants were administered; the language composition of each group is
% currently not reported anywhere in the thesis.
% TODO(Enrico): report completion/dropout, or state that the figure is not available and why.
...
The code was independent of whether the scene was grasped: several participants who correctly described the opposing directions still read the curve's height as speed, carrying both the grasped verdict and the misconception.\footnote{Because the experimental condition also had lower mean data-visualisation familiarity (\S\ref{sec:res-sample}), condition and familiarity are partly confounded at this sample size. Note that the two splits produce the same counts (ten of thirteen against four of twelve in both), which indicates that they are close to the same partition of the sample rather than two independent views of it. The two panels of Figure~\ref{fig:misconception} should therefore be read as one confounded observation, and their joint reading is exploratory. This point is taken up in Chapter~\ref{ch:discussion}.} Three participants explicitly distinguished cumulative transport from velocity and did not carry the code; two of them also asked, unprompted, that the axis be labelled to show which quantity it represented.

% -------------------------------------------------------------------------
\section{Confidence and Correctness}
\label{sec:res-confidence}

Mean confidence was higher for grasped responses than for non-grasped ones in every scene, but the size of the gap varied (Figure~\ref{fig:confidence}). It was widest in Scene~II (6.33 for the three grasped responses against 4.27 for the remaining twenty-two) and Scene~III (6.32 across twenty-two grasped against 4.67 across the three that were not). It was narrowest in Scene~I, where the eight non-grasped responses still averaged 6.0 against 6.82 for the seventeen grasped ones. The Scene~II and Scene~III gaps each rest on a subgroup of three and should be read accordingly.

The pre-specified definition of a confidently-wrong response was a confidence of at least seven together with a \emph{not\_grasped} verdict. Under this definition, no such case appeared in Scene~I. A wider reading, which also admits \emph{partial} verdicts, identified four Scene~I participants (P02, P04, P12 and P17) who rated their confidence at seven or above while coding only partial. Each of these four carried the streamfunction misconception.\footnote{This wider reading is reported as a post-hoc observation and is kept separate from the pre-specified definition, which returns no cases in Scene~I. The four participants are marked in Figure~\ref{fig:confidence}.} Two further cases appear in Scene~II under the same wider reading, one in each condition, and none in Scene~III.
...
\caption[Recurring emergent codes by condition]{The three recurring emergent codes by condition. The first two are mirror images of one another; the third runs against the experiential condition. Post-hoc codes that emerged during coding; not part of the pre-specified grid.}
\label{fig:emergent}
\end{figure}
...
\caption[All emergent codes by condition]{The complete list of emergent codes by condition. Bold marks the three recurring codes. All are post-hoc and were not part of the pre-specified grid.}
